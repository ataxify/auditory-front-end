...
switch upper(select)
    case 'SURREY_A'
        % Root directory
        rootHRTF = [fullfile(fileparts(which(mfilename)),'..',...
            'HRTF_SURREY','Anechoic','16kHz'),filesep];

        % HRTF Name
        nameHRTF = ['CortexBRIR_0s_',num2str(azim),'deg_16k.wav'];
        
        % HATS database 
        [hrtf,fsRef] = audioread([rootHRTF,nameHRTF]);
        
    case 'SURREY_ROOM_A'
        % Root directory
        rootHRTF = [fullfile(fileparts(which(mfilename)),'..',...
            'HRTF_SURREY','Room_A','16kHz'),filesep];
    ...
    case 'SURREY_ROOM_B'
        % Root directory
        rootHRTF = [fullfile(fileparts(which(mfilename)),'..',...
            'HRTF_SURREY','Room_B','16kHz'),filesep];
    ...
    case 'SURREY_ROOM_C'
        % Root directory
        rootHRTF = [fullfile(fileparts(which(mfilename)),'..',...
            'HRTF_SURREY','Room_C','16kHz'),filesep];
    ...
    case 'SURREY_ROOM_D'
        % Root directory
        rootHRTF = [fullfile(fileparts(which(mfilename)),'..',...
            'HRTF_SURREY','Room_D','16kHz'),filesep];
